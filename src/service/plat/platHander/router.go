...
func AddRouter(c *gin.Context) {
	...
	if err == nil {
		...
		// 执行创建
		service.JsonRes(c, platServer.AddRouter(traceID, req))
	}
}
...
func PageRouter(c *gin.Context) {
	...
	if err == nil {
		...
		// 执行查询
		service.JsonRes(c, platServer.PageRouter(traceID, req))
	}
}
...
func GetRouter(c *gin.Context) {
	...
	if err == nil {
		...
		// 执行查询
		service.JsonRes(c, platServer.GetRouter(traceID, req))
	}
}
...
func EditRouter(c *gin.Context) {
	...
	if err == nil {
		...
		// 执行创建
		service.JsonRes(c, platServer.EditRouter(traceID, req))
	}
}
...
func DelRouter(c *gin.Context) {
	...
	if err == nil {
		req := reqObj.(*baseModel.IdReq)
		// 执行查询
		service.JsonRes(c, platServer.DelRouter(traceID, req))
	}
}
